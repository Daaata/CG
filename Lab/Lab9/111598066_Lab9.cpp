...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("Lab 09: Rasterization - Animation");
	...
	SetupRC();
	glutTimerFunc(33, HalfSpaceTest, 1);
...
void HalfSpaceTest(int i)
{
	...
	for (int i = 0; i < ck_x.size(); i++) //Base on Crow’s Algorithm
	{
		int t;
		//glColor3f(0.12, 0.36, 0.36);
		//glBegin(GL_LINES);
		...
		drawbox[ck_x[t] + 20][ck_y[t] + 20] = 1;
		MidpointAlgorithm(ck_x[i], ck_y[i], ck_x[t], ck_y[t]);
	}
...
void MidpointAlgorithm(int get_x0, int get_y0, int get_x1, int get_y1)
{
	...
	if (get_x1 - get_x0 != 0)
	{
		...
		//cout << ck_m[0] * get_x1 - get_y1 + ck_c[0] << endl;
